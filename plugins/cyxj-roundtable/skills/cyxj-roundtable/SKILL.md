...
读取模板：
```
Read: ${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/templates/briefing.md
```
...
- `${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/seats/02-peer.md` → 严苛同行
- `${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/seats/03-investor.md` → 带偏见投资人
- `${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/seats/04-historian.md` → 历史学家
- `${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/seats/05-future-self.md` → 5 年后后悔的你
- `${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/seats/06-ally.md` → 同盟者

每次 Agent 调用参数：
- `subagent_type`: `general-purpose`
- `model`: `opus`（默认 Opus 4.7；dry-run 模式例外，见下文）
...
[你的角色]
<对应 ${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/seats/0N-*.md 文件的完整内容>
...
读 `${CLAUDE_PLUGIN_ROOT}/skills/cyxj-roundtable/templates/record.md` 模板，按格式填充所有字段。
...
写入步骤：
1. 先确认这是真实的 Obsidian 库：`~/obsidian/灵感库/.obsidian` 或
   `~/obsidian/.obsidian` 目录存在。不存在则问用户库路径；
   用户不管时，降级写到当前工作目录，并明确告知"决策记录写在了 cwd，
   不是 Obsidian 库"。**不要在非库路径上静默 `mkdir -p` 造假目录**
2. 库确认后，`决策记录/` 子目录不存在则 `mkdir -p`
3. 检查文件是否重名，重名加 `-2`、`-3`
4. 写文件
5. 输出文件绝对路径给用户
...
| 故障 | 处理 |
|---|---|
| 某席 subagent 迟迟未返回 | 报告"第 X 席迟迟未返回"，问用户：等 / 跳过 / 重启该席 |
| 某席输出明显跑偏 | 用户说"第 X 席重开" → 单独重启该 subagent |
| Obsidian 库校验失败 | 按阶段 5 写入步骤：问用户库路径，或降级写到 cwd 并告知 |
| 文件名重复 | 自动加 `-2`、`-3` |
...
触发方式：用户说「圆桌 dry-run」时，主持人在简报标题加 `[DRY-RUN]`。
